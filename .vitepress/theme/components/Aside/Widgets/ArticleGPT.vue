...
<template>
  <div v-if="frontmatter.articleGPT" class="article-gpt s-card">
    ...
    <div class="content s-card">
      <span class="text">{{ abstractData === "" ? t('articleGPT.loading') : abstractData }}</span>
      <span v-if="loading" class="point">|</span>
    </div>
    <div class="meta">
      <span class="tip">{{ t('articleGPT.tip') }}</span>
    </div>
  </div>
</template>
...
const typeWriter = (text = null) => {
  try {
    ...
    loading.value = false
    abstractData.value = t('articleGPT.loading')
...
const showOther = () => {
  ...
  const text = t('articleGPT.fake_gpt_intro')
  showIndex.value = 0
  loading.value = true
  abstractData.value = ""
  if (!showType.value) {
